...
def test_data_message_creation():
    """Test that DataMessage correctly stores trace_id, session_id, event_type, and data."""
    msg = DataMessage(
        trace_id="trace-1",
        session_id="sess-1",
...
def test_cancel_action_and_states_exist():
    """Test that ActionType.CANCEL_TASK and AgentState cancellation states have correct values."""
...
def test_resume_requires_status_or_content():
    """Test that ResumeCommand raises ValueError when neither status nor content is provided."""
...
def test_ask_agent_command_round_trip_from_wire_dict():
    """Test that AskAgentCommand can be serialized to dict and decoded back correctly."""
...
def test_command_to_redis_payload_serializes_wire_dict():
    """Test that command.to_redis_payload produces a valid Redis payload with serialized data."""
    command = AskAgentCommand(
        header=MessageHeader(
...
def test_custom_command_can_be_registered_and_decoded():
    """Test that custom command types can be dynamically registered and decoded via command_from_dict."""
