...
def mock_history_provider():
    with patch(
        "byclaw_gateway_sdk.core.history.HistoryProvider.save_message",
...
@pytest.mark.asyncio
async def test_context_accumulates_and_flushes(mock_redis, mock_history_provider):
    ...
    # 此时不应触发保存
    mock_history_provider.assert_not_called()

    # 发送流结束标识
    await context.emit_chunk("", event_type=EventType.APP_STREAM_RESPONSE.value)

    # 验证保存内容
    mock_history_provider.assert_awaited_once()
    args, kwargs = mock_history_provider.call_args
...
async def test_worker_saves_user_and_assistant_history(
    mock_redis, mock_history_provider
):
    ...
    assert mock_history_provider.call_count == 2

    # 检查用户消息
    user_call = mock_history_provider.call_args_list[0]
    assert user_call.kwargs["role"] == "user"
    assert user_call.kwargs["content"] == "User Question"

    # 检查助手回复 (由 MockWorker 积累的 "Hello World")
    assistant_call = mock_history_provider.call_args_list[1]
    assert assistant_call.kwargs["role"] == "assistant"
    assert assistant_call.kwargs["content"] == "Hello World"


@pytest.mark.asyncio
async def test_duplicate_save_prevention(mock_redis, mock_history_provider):
    ...
    # 应该只调用了一次保存
    assert mock_history_provider.call_count == 1


@pytest.mark.asyncio
async def test_in_memory_storage_isolation():
    """验证 InMemoryHistoryStorage 的多会话隔离"""
    from byclaw_gateway_sdk.core.history import InMemoryHistoryStorage
...
@pytest.mark.asyncio
async def test_history_provider_backend_switch(mock_redis):
    """验证 HistoryProvider 动态切换后端逻辑"""
    from byclaw_gateway_sdk.core.history import BaseHistoryStorage
    ...
    HistoryProvider.set_storage(custom_storage)

    # 触发保存
    await HistoryProvider.save_message("s4", "assistant", "test")
    assert custom_storage.saved is True

    # 恢复默认，避免影响其他测试
    from byclaw_gateway_sdk.core.history import InMemoryHistoryStorage

    HistoryProvider.set_storage(InMemoryHistoryStorage())
