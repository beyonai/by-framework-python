...
class LoggingConfig:
    ...
    level: str = "INFO"
    use_json: bool = False
    log_file: Optional[str] = "gateway-sdk.log"
...
@dataclass(frozen=True)
class SDKConfig:
    """Main SDK configuration combining all sub-configs."""
    ...
    def from_env(cls) -> "SDKConfig":
        """Load complete SDK configuration from environment variables."""
        return cls(
...
def get_config() -> SDKConfig:
    """Get the global SDK configuration, loading from environment if needed."""
    global _config
    if _config is None:
...
def init_config(config: SDKConfig) -> None:
    """Initialize the global SDK configuration (for testing or custom config)."""
    global _config
    _config = config
