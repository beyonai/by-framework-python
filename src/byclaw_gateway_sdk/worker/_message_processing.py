...
async def inject_history(command: AskAgentCommand) -> None:
    """
    Inject session history into command payload.

    This modifies the command in-place.
    """
    history = await HistoryProvider.get_session_history(command.header.session_id)
    command.extra_payload["history"] = history
...
async def process_command(
    ...
    # Inject history for AskAgentCommand
