...
class AgentConfigManager:
    ...
    def add_config(self, config: AgentConfig) -> bool:
        ...
        if config.agent_id in self._configs:
            ...
            existing_config = self._configs[config.agent_id]
    ...
    def search_configs(
        ...
        if name:
            ...
            results = [
                c for c in results if name_lower in c.name.lower() or name_lower in c.agent_id.lower()
            ]
        ...
        if callback_type:
            results = [
                c for c in results if callback_type in c.callbacks and c.callbacks[callback_type]
            ]
